...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    $current = mysqli_query($conn, "SELECT nama FROM destinasi WHERE id = $id LIMIT 1");
    $current_row = $current ? mysqli_fetch_assoc($current) : null;

    mysqli_query($conn, "DELETE FROM gallery WHERE ref_id = $id AND ref_type = 'destinasi'");

    mysqli_query($conn, "DELETE FROM destinasi WHERE id = $id");

    if ($current_row && isset($current_row['nama'])) {
        log_audit('delete_destinasi', "Menghapus destinasi \"{$current_row['nama']}\".", $id, $current_row['nama']);
    }
...
if (isset($_GET['action']) && $_GET['action'] == 'toggle' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $query = mysqli_query($conn, "SELECT nama, status FROM destinasi WHERE id = $id");
    $data = mysqli_fetch_assoc($query);
    $new_status = ($data['status'] == 'aktif') ? 'nonaktif' : 'aktif';

    mysqli_query($conn, "UPDATE destinasi SET status = '$new_status' WHERE id = $id");
    if ($data && isset($data['nama'])) {
        $status_msg = $new_status === 'aktif' ? 'diaktifkan' : 'dinonaktifkan';
        log_audit('toggle_destinasi', "Status destinasi \"{$data['nama']}\" $status_msg.", $id, $data['nama']);
    }
    $_SESSION['success_message'] = "Status destinasi berhasil diubah!";
    redirect('destinasi.php');
}
...
if ($exporting) {
    ...
    $filename = "destinasi-" . date('Ymd-His') . "." . ($format === 'json' ? 'json' : 'csv');
    $export_query = "SELECT id, nama, deskripsi, deskripsi_lengkap, lokasi, gambar_utama, tiket_masuk, fasilitas, jam_buka, dos, donts, maps_embed, status, created_at, updated_at FROM destinasi $where_sql ORDER BY created_at DESC";
    ...
    $action = $format === 'json' ? 'Download JSON Destinasi' : 'Download CSV Destinasi';
    $target_label = "Destinasi ($format_label)";
    log_audit($action, "Export destinasi ($count data, format $format).", null, $target_label);
    ...
    fputcsv($output, ['id', 'nama', 'deskripsi', 'deskripsi_lengkap', 'lokasi', 'gambar_utama', 'tiket_masuk', 'fasilitas', 'jam_buka', 'dos', 'donts', 'maps_embed', 'status', 'created_at', 'updated_at']);
    foreach ($rows as $row) {
        fputcsv($output, $row);
    }
    fclose($output);
...
$total_rows = 0;
$total_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM destinasi $where_sql");
if ($total_query) {
...
$query = "SELECT * FROM destinasi $where_sql ORDER BY $sort $order LIMIT $offset, $per_page";
    ...
    <div class="admin-wrapper">
        ...
        <div class="admin-content">
            <div class="admin-header">
                ...
                <a href="destinasi-form.php" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Destinasi</a>
            </div>
            ...
            <div class="filter-section filter-with-export">
                <form method="GET" class="filter-form">
                    <div class="filter-group">
                        ...
                        <a href="destinasi.php" class="btn btn-secondary"><i class="fas fa-redo"></i> Reset</a>
                    </div>
                </form>
                ...
                <table class="data-table content-table">
                    <thead>
                        <tr>
                            ...
                            <th>Gambar</th>
                            <th><a href="<?php echo sortUrl('nama'); ?>" class="sort-link">Nama Destinasi
                            ...
                            <tr>
                                ...
                                <td>
                                    <img src="../<?php echo htmlspecialchars($row['gambar_utama']); ?>"
                                        alt="<?php echo htmlspecialchars($row['nama']); ?>" class="table-image">
                                </td>
                                <td><?php echo htmlspecialchars($row['nama']); ?></td>
                                <td><?php echo htmlspecialchars($row['lokasi']); ?></td>
                                <td><?php echo htmlspecialchars($row['tiket_masuk']); ?></td>
                                <td>
                                    <span
                                ...
                                <td class="action-buttons">
                                    <a href="gallery.php?ref_id=<?php echo $row['id']; ?>&ref_type=destinasi"
                                        class="btn btn-sm btn-gallery" title="Kelola Galeri">
                                        <i class="fas fa-images"></i>
                                    </a>
                                    <a href="destinasi-form.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-info"
                                        title="Edit">
                                    ...
                                    <a href="?action=toggle&id=<?php echo $row['id']; ?>"
                                        ...
                                        title="Toggle Status" data-confirm="Ubah status destinasi ini?">
                                    ...
                                    <a href="?action=delete&id=<?php echo $row['id']; ?>"
                                        ...
                                        data-confirm="Yakin ingin menghapus destinasi ini?">
